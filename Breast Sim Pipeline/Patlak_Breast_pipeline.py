...
class DCE(nn.Module):
    ...
    def _param_to_conc_patlak(self, x):
        ...
        # Add a 3-second delay to K_time
        delay_steps = int(3 / dt[-1])  # 3 seconds delay
        K_time_delayed = torch.cat((torch.zeros(delay_steps, device=K_time.device), K_time))[:len(K_time)]

        mult = torch.stack((K_time_delayed, aifci_tens), 1)  # [1502, 2]
    ...
    def _param_to_conc_tcm(self, x):
        ...
        # avoid detach numpy here because it messes up the gradients for fp and PS
    def fft_convolve_time_only(self, aif_map, kernel):
        ...
        for i in range(aif_map.shape[1]):
            for j in range(aif_map.shape[2]):
                ...
                # Perform the 1D convolution using PyTorch
    ...
    def convolve_aif_kernel(self, aif, kernel):
        # Determine FFT lengths
        fft_length = aif.size(0)

        # Perform FFT of the AIF along the time dimension
        aif_fft = torch.fft.rfft(aif, n=fft_length)

        # Perform FFT of the kernel along the time dimension
        kernel_fft = torch.fft.rfft(kernel, n=fft_length, dim=0)

        # Expand dimensions of AIF to match kernel dimensions for broadcasting
        aif_fft_expanded = aif_fft[:, None, None]

        # Multiply in the frequency domain
        convolved_fft = aif_fft_expanded * kernel_fft

        # Perform inverse FFT to get back to time domain
        convolved = torch.fft.irfft(convolved_fft, n=fft_length, dim=0)
    ...
    def _param_to_conc_tcm_SnB(self, x):
        ...
        he_sum = torch.sum(he_clone, dim=0, keepdim=True)
        hp_sum = torch.sum(hp_clone, dim=0, keepdim=True)

        # Ensure no zero values in sums to avoid division by zero
        epsilon = 1e-10  # Small value to prevent division by zero
        he_sum = torch.clamp(he_sum, min=epsilon)
        hp_sum = torch.clamp(hp_sum, min=epsilon)
        ...
        ce = self.fft_audio_convolve(aifci_tens, he)
        cp = self.fft_audio_convolve(aifci_tens, hp)
...
data = sio.loadmat('DRO Runs/DRO_RUN_5.mat')
...
parmap_tens = parmap_tens.permute(2, 0, 1).unsqueeze(1)
...
optimizer = optim.Adam([x], lr=0.001, amsgrad=True)
...
for epoch in range(500):
    ...
    # here verify that param map gradients are non zero before the optimizer step?
...
outfilestr = 'Patlak_Run5_wDelay.h5'
