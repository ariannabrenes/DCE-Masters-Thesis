...
def arterial_input_function(time, inj_delay = 40/60,
                            A=[0.309, 0.330],
                            T=[0.17046, 0.365],
                            sigma=[0.0563, 0.132],
                            alpha=1.050,
...
def Patlak(ishape, sample_time, device=sp.cpu_device):
    ...
    References:
        * Patlak C, Blasberg RG, Fenstermacher JD.
          Graphical Evaluation of Blood-to-Brain Transfer Constants from Multiple-Time Uptake Breast Data.
...
class DCE(nlop.Nlop):
    ...
    # # adj_input represents the gradient of the loss with respect to the output of the _forward method
    # # adj represents the gradient of the loss with respect to the input x
    # #  The Jacobian matrix is computed using the _get_Jacobian method, and then the element-wise multiplication between
    #     #  the Jacobian and adj is performed to obtain the gradient dx.
    #     #  Finally, dx is returned as the gradient of the loss with respect to the input.
    #
    # def _backward(self, input, adj_input):
    #     xp = self.device.xp
    #
    #     with self.device:
    #         # Convert input and adj_input to device
    #         x = _array_to_device(input, device=self.device)
    #
    #         # Compute the adjoint of the input
    #         adj = self._adjoint(x, adj_input)
    #
    #         # Compute the Jacobian matrix
    #         Jacobian = self._get_Jacobian(x)
    #
    #         # Element-wise multiplication of Jacobian with adj_input
    #         dx = Jacobian * adj
    #
    #         # Sum along the first axis to obtain the gradient
    #         grad = xp.sum(dx, axis=0)
    #
    #         return grad
